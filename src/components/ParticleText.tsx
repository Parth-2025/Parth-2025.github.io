// Source: https://reactbits.dev/r/ParticleText-TS-TW.json
// Adapted styles and stopped idle rendering for this portfolio.
  ...
  useEffect(() => {
    ...
    const render = (now: number): void => {
      ...
      if (gathering && complete) {
        gathering = false;
        particles.forEach(p => { p.x = p.targetX; p.y = p.targetY; });
        ctx.clearRect(0, 0, width, height);
        particles.forEach(drawParticle);
      }

      animationFrame = (!reducedMotion && (gathering || idleDrift > 0 || pointerRepel > 0)) ? window.requestAnimationFrame(render) : null;
    };
